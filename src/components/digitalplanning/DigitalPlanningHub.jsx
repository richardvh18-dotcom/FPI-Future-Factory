...
import {
  ...
  Users,
  LogOut,
  CalendarRange, // Icoon voor de Planner tegel
} from "lucide-react";

import { useAdminAuth } from "../../hooks/useAdminAuth";
import TeamleaderHub from "./TeamleaderHub";
import WorkstationHub from "./WorkstationHub";
import PlannerHub from "./PlannerHub"; // NIEUW: Importeer de losse PlannerHub

/**
 * DigitalPlanningHub V10.4 (Met losse PlannerHub)
...
const DigitalPlanningHub = ({ onBack }) => {
  ...
  const handleExitStation = () => {
    ...
    // Als we uit de Planner view komen, ga terug naar het hoofdmenu
  ...
  if (!activeDept && !currentStation) {
    return (
      <div className="flex flex-col h-screen w-full bg-slate-50 items-center justify-center p-6 overflow-hidden">
        <div className="w-full max-w-6xl animate-in fade-in zoom-in duration-500 text-center">
  ...
  // Als de Planner is gekozen, maar er is nog geen specifiek station aangeklikt (om in te kijken)
  if (activeDept === "PLANNER" && !currentStation) {
    return (
      <PlannerHub
        onExit={handleExitStation}
        // Zorg dat als de planner op een machine-tegel klikt, hij naar de WorkstationHub gaat
        onEnterWorkstation={(stationId) => setCurrentStation({ id: stationId })}
      />
    );
  }

  // Filter de stations voor de gekozen afdeling (alleen als NIET planner)
  const filteredStations = ALL_STATIONS.filter(
    (s) => s.dept === "ALL" || s.dept === activeDept
  );

  // STAP 2: KIES STATION (BINNEN AFDELING)
  // Dit slaan we over als we in 'PLANNER' modus zitten (zie if-blok hierboven)
  if (!currentStation) {
    const deptInfo = DEPARTMENTS.find((d) => d.id === activeDept);

    return (
      <div className="flex flex-col h-screen w-full bg-slate-50 items-center justify-center p-6 overflow-hidden">
        <div className="w-full max-w-7xl animate-in fade-in slide-in-from-right-8 duration-500">
          <div className="mb-12 text-center relative">
            <button
              onClick={() => setActiveDept(null)}
              className="absolute left-0 top-1/2 -translate-y-1/2 p-3 bg-white rounded-full text-slate-400 hover:text-blue-600 border border-slate-200 shadow-sm hover:shadow-md transition-all"
            >
              <ArrowLeft size={24} />
            </button>
            <h1 className="text-4xl font-black text-slate-800 uppercase italic tracking-tighter mb-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 justify-center">
  ...
  // STAP 3: TOON HUB (TEAMLEAD OF WORKSTATION)

  if (currentStation.id === "TEAMLEAD") {
    // Voor normale afdelings-teamleads
    const scope =
  ...
  // Anders -> WorkstationHub (voor zowel operators als managers die doorklikken)
  return (
    <WorkstationHub
